...
use crate::service::caching::{TTL_ANALYSTS, analysts_key};
use actix_web::{HttpResponse, Result, web};
use finance_query_core::models::analysts::{
    AnalysisType, EarningsEstimate, EarningsEstimateResponse, EarningsHistoryItem,
    EarningsHistoryResponse, PriceTarget, PriceTargetsResponse, RecommendationData,
...
pub async fn get_recommendations_handler(
    ...
    app_state
        .cache_service
        .set(&cache_key, &response_json, TTL_ANALYSTS)
        .await;
...
pub async fn get_upgrades_downgrades_handler(
    ...
    let upgrades_downgrades: Vec<UpgradeDowngrade> = serde_json::from_value(
        data.get("upgrades_downgrades")
            .ok_or_else(|| {
                actix_web::error::ErrorInternalServerError("No upgrades_downgrades data")
            })?
            .clone(),
    )
    .map_err(|e| {
        actix_web::error::ErrorInternalServerError(format!(
            "Failed to parse upgrades_downgrades: {}",
            e
        ))
    })?;
...
pub async fn get_price_targets_handler(
    ...
    let data =
        analysts::get_analysis_data(&app_state.yahoo_client, &symbol, AnalysisType::PriceTargets)
            .await
            .into_web_result()?;
    ...
    .map_err(|e| {
        actix_web::error::ErrorInternalServerError(format!("Failed to parse price_targets: {}", e))
    })?;
...
pub async fn get_earnings_estimate_handler(
    ...
    .map_err(|e| {
        actix_web::error::ErrorInternalServerError(format!(
            "Failed to parse earnings_estimate: {}",
            e
        ))
    })?;
...
pub async fn get_revenue_estimate_handler(
    ...
    .map_err(|e| {
        actix_web::error::ErrorInternalServerError(format!(
            "Failed to parse revenue_estimate: {}",
            e
        ))
    })?;
...
pub async fn get_earnings_history_handler(
    ...
    .map_err(|e| {
        actix_web::error::ErrorInternalServerError(format!(
            "Failed to parse earnings_history: {}",
            e
        ))
    })?;
