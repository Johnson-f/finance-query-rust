...
/// Sectors WebSocket endpoint - streams sector performance data
pub async fn sectors_handler(
    req: HttpRequest,
    body: web::Payload,
    app_state: web::Data<AppState>,
    manager: web::Data<ConnectionManagerAddr>,
) -> Result<HttpResponse> {
    info!("WebSocket sectors connection established");
    
    let channel = "sectors".to_string();
    ...
    let task = tokio::spawn(async move {
        loop {
            ...
            let data_result = fetch_sectors_data(&fetch).await;
            ...
            match data_result {
                ...
                Err(e) => {
                    error!("Failed to fetch sectors data: {}", e);
                }
            }
