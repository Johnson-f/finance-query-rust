...
pub fn configure_routes(cfg: &mut actix_web::web::ServiceConfig) {
    cfg.service(
        web::scope("/v1")
            ...
            .route("/news", web::get().to(news::get_general_news_handler))
            .route("/financials/{symbol}", web::get().to(financials::get_financials_handler))
            .route("/earnings/{symbol}/calls", web::get().to(earnings::get_earnings_calls_handler))
            .route("/earnings/{symbol}/transcript/{event_id}", web::get().to(earnings::get_earnings_transcript_handler)),
    )
