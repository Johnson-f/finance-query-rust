...
use crate::service::caching::{TTL_HOLDERS, holders_key};
...
pub async fn get_major_holders_handler(
    ...
    let data = holders::get_holders_data(&app_state.yahoo_client, &symbol, HolderType::Major)
        .await
        .into_web_result()?;
    ...
    // Cache the result
    let response_json: Value = serde_json::to_value(&response).map_err(|e| {
        actix_web::error::ErrorInternalServerError(format!("Failed to serialize response: {}", e))
    })?;
    app_state
        .cache_service
        .set(&cache_key, &response_json, TTL_HOLDERS)
        .await;
...
pub async fn get_institutional_holders_handler(
    ...
    let data =
        holders::get_holders_data(&app_state.yahoo_client, &symbol, HolderType::Institutional)
            .await
            .into_web_result()?;

    let response = InstitutionalHoldersResponse {
        symbol: data.symbol,
        holders: data.institutional_holders.unwrap_or_default(),
    };
...
pub async fn get_mutualfund_holders_handler(
    ...
    let data = holders::get_holders_data(&app_state.yahoo_client, &symbol, HolderType::MutualFund)
        .await
        .into_web_result()?;

    let response = MutualFundHoldersResponse {
        symbol: data.symbol,
        holders: data.mutualfund_holders.unwrap_or_default(),
    };
...
pub async fn get_insider_transactions_handler(
    ...
    let response = InsiderTransactionsResponse {
        symbol: data.symbol,
        transactions: data.insider_transactions.unwrap_or_default(),
    };
...
pub async fn get_insider_purchases_handler(
    ...
    let response = InsiderPurchasesResponse {
        symbol: data.symbol,
        summary: data.insider_purchases.ok_or_else(|| {
            actix_web::error::ErrorInternalServerError("No insider purchases data")
        })?,
    };
...
pub async fn get_insider_roster_handler(
    ...
    let data =
        holders::get_holders_data(&app_state.yahoo_client, &symbol, HolderType::InsiderRoster)
            .await
            .into_web_result()?;

    let response = InsiderRosterResponse {
        symbol: data.symbol,
        roster: data.insider_roster.unwrap_or_default(),
    };
