...
pub async fn get_sectors(fetch_client: &Arc<FetchClient>) -> Result<Vec<MarketSector>, YahooError> {
    ...
    let sectors: Vec<MarketSector> = serde_json::from_str(trimmed).map_err(|e| {
        warn!(
...
pub async fn get_sector_for_symbol(
    ...
    let sector: MarketSector = serde_json::from_str(trimmed).map_err(|e| {
        warn!(
            "Failed to parse JSON from {} for symbol {}: {}. Response preview: {}",
            url,
            symbol,
            e,
            trimmed.chars().take(200).collect::<String>()
...
pub async fn get_sector_details(
    ...
    info!(
        "Fetching detailed sector data for: {} from API",
        sector.as_str()
    );
    ...
    if trimmed.is_empty() {
        warn!(
            "Empty response received from {} for sector {}",
            url, sector_name
        );
        return Err(YahooError::ParseError(format!(
