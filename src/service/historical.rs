...
pub async fn get_historical(
    ...
    if !is_natively_supported(&interval) {
        ...
        if let Some(target_minutes) = interval_minutes(&interval) {
            warn!("Interval '{}' is not natively supported by Yahoo Finance. Resampling from 1m data.", interval.as_str());
            return get_historical_resampled(yahoo_client, symbol, time_range, target_minutes).await;
        } else {
...
async fn get_historical_max_weekly(
    ...
    info!("Fetching max range with 1wk interval for {}, using optimized chunking strategy", symbol);
    ...
    loop {
        ...
        debug!("Fetching chunk {} for {} (weekly): {} to {} ({})", 
               chunk_count + 1, 
               symbol, 
               period1, 
               period2,
    ...
    info!("Successfully fetched {} chunks for {} (weekly, total {} data points)", 
          chunk_count, symbol, all_data.len());
...
async fn get_historical_resampled(
    ...
    info!("Resampling historical data for {}: fetching 1m data and aggregating to {}m intervals", symbol, target_minutes);
    ...
    if one_minute_data.data.is_empty() {
        return Ok(HistoricalResponse { data: HashMap::new() });
    }
    
    // Convert to sorted vector of (timestamp, data) tuples
    let mut sorted_data: Vec<(i64, HistoricalData)> = one_minute_data.data
        .into_iter()
    ...
    for (timestamp, data) in sorted_data {
        ...
        let bucket_start = (timestamp / (target_minutes as i64 * 60)) * (target_minutes as i64 * 60);
    ...
    info!("Resampled {} 1m data points to {} {}m intervals", 
          data_count, resampled.len(), target_minutes);
...
fn aggregate_bucket(
    ...
    let volume: i64 = bucket.iter()
        .map(|(_, data)| data.volume)
        .sum();
...
pub fn calculate_indicators(
    ...
    let mut price_points: Vec<PricePoint> = historical.data.iter()
        .filter_map(|(ts_str, data)| {
            // Try parsing as RFC3339 first, then fallback to Unix timestamp string
            let timestamp = chrono::DateTime::parse_from_rfc3339(ts_str)
                .map(|dt| dt.timestamp())
