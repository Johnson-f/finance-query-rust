...
impl PriceBufferManager {
    ...
    pub async fn add_price(&self, buffer_key: &str, price: f64, is_daily: bool, is_weekly: bool) -> bool {
        ...
        let buffer = buffers.entry(buffer_key.to_string()).or_insert_with(Vec::new);
    ...
    pub async fn initialize_from_historical(&self, symbol: &str, prices: Vec<PricePoint>) {
        ...
        let prices_to_store: Vec<PricePoint> = if prices_len > self.max_size {
            prices.into_iter().skip(prices_len - self.max_size).collect()
        } else {
            prices
        };
