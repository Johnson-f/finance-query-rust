...
async fn fetch_movers(
    ...
    let response = yahoo_client.make_request(url, Some(&params)).await?;
    ...
    let quotes = data
        ...
        .ok_or_else(|| YahooError::ParseError("No quotes found in movers response".to_string()))?;
    ...
    for item in quotes {
        ...
        let price = get_fmt(item, "regularMarketPrice").unwrap_or_else(|| "0.0".to_string());

        let change = get_fmt(item, "regularMarketChange").unwrap_or_else(|| "0.0".to_string());

        let percent_change =
            get_fmt(item, "regularMarketChangePercent").unwrap_or_else(|| "0.0%".to_string());
