...
pub async fn scrape_news_for_quote(
    ...
    let news: Vec<News> = serde_json::from_str(&response_text).map_err(|e| {
        YahooError::ParseError(format!("Failed to parse news JSON for {}: {}", symbol, e))
    })?;
    ...
    info!(
        "Successfully fetched {} news items for {}",
        news.len(),
        symbol
    );
    Ok(news)
...
pub async fn scrape_general_news(fetch_client: &Arc<FetchClient>) -> Result<Vec<News>, YahooError> {
