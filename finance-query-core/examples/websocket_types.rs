...
use finance_query_core::{
    FetchClient, MarketHours, MovingAverageUpdate, QuotesUpdate, SimpleQuote, YahooAuthManager,
    YahooFinanceClient,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 1..=3 {
        ...
        for quote in &update.quotes {
            println!(
                "  {} ({}): ${} | {} ({})",
                quote.symbol, quote.name, quote.price, quote.change, quote.percent_change
            );
        }
        ...
        println!(
            ...
            serde_json::to_string(&update)?.len()
        );
        println!();
    }
    ...
    println!(
        "Market Status: {} ({})",
        market_hours.status,
        market_hours.reason.as_deref().unwrap_or("")
    );
    ...
    println!(
        "{} {} ({}): {}",
        ma_update.symbol, ma_update.indicator_type, ma_update.period, ma_update.value
    );
